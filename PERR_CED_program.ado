...
capture program drop PERR_CED_Program
program define PERR_CED_Program, rclass

version 15.1
 syntax [, n(integer 600) var(real 0.5) beta0(real -1.5) hr(real 2.0)   ///
		    ka(real 0.8) ksi(real 1.0) maxE(integer 200)  ]
...
local j = `i'-1
gen v = runiform()
gen rate = `ka'*sup^(`ka'-1)*lam*exp(cond(`beta'>0, `beta',0)+cond(`ksi'>0, `ksi',0)*log(eventN`j'+1))

gen u = rexponential(1/rate)  /* scale = 1/rate  */
replace t_star = t_star + u

gen t`i' = t`j'
replace t`i' = t_star if v <= `ka'*(t_star)^(`ka'-1)*lam*exp(`beta'*(ct<exit)*(t_star>ct)+`ksi'*log(eventN`j'+1))/rate
